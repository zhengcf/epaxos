...
func (r *Replica) stopAdapting() {
	...
	log.Println("------zcf before stopAdapint preferredorder: ",r.PreferredPeerOrder)
	for i := 0; i < r.N-1; i++ { // i = 0
		min := i
		for j := i + 1; j < r.N-1; j++ { // i = 0; min = 0; j = 1
	...
	log.Println("------zcf after stopAdapint preferredorder: ",r.PreferredPeerOrder)
}
...
func (r *Replica) run() {
    ...
    dlog.Println("-----zcf id: ", r.Id)
    dlog.Println("-----zcf PreferredPeerOrder before :", r.PreferredPeerOrder)
	if r.Id == 0 {
	...
	dlog.Println("-----zcf PreferredPeerOrder after :", r.PreferredPeerOrder)

	slowClockChan = make(chan bool, 1)
	fastClockChan = make(chan bool, 1)
	go r.slowClock()  //150ms
